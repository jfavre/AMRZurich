...
public:
  ...
  ScaleOption GetScaleChoice() { return this->ScaleChoice;};
...
protected:
  ...
  void CloseHDF5File(const char *funcName);
  int  OpenHDF5File(const char *funcName);
  void WriteVTIFile(const char *fileName, adG_grid &grid, double *spacing);
